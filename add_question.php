...
// Attempt to include db.php, otherwise use inline connection (fallback)
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $conn) {
    ...
    $pattern_code = !empty($_POST['pattern_code']) ? $_POST['pattern_code'] : NULL;
    ...
    $sql = "INSERT INTO questions (category_id, question, pattern_code, option1, option2, option3, option4, answer) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
...
<head>
    ...
    <title>Add Question - LogicLoom</title>
...
<body class="bg-slate-100 dark:bg-slate-900 text-slate-800 dark:text-slate-100 relative overflow-x-hidden transition-colors duration-300">
        ...
        <main class="flex-1 flex flex-col h-screen overflow-y-auto">
            ...
            <header class="glass-panel sticky top-0 z-10 px-6 py-4 flex justify-between items-center shadow-sm transition-colors duration-300">
                <div class="flex items-center gap-4">
                    ...
                    <h2 class="text-xl font-semibold text-slate-800 dark:text-white">Question Manager</h2>
                </div>
            ...
            <div class="p-6 lg:p-10 max-w-4xl mx-auto w-full">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg dark:shadow-sm border border-slate-200 dark:border-slate-700 p-8">
                    ...
                    <div class="flex items-center gap-3 mb-6">
                        ...
                        <h2 class="text-xl font-bold text-slate-800 dark:text-white">Add New Question</h2>
                    </div>
                    ...
                    <form method="POST" class="space-y-6">
                        ...
                        <!-- Question Text -->
                        <div>
                            <label class="block mb-2 text-sm font-medium text-slate-600 dark:text-slate-400">Question Text</label>
                            <textarea name="question" required placeholder="Type the question here..."
                        ...
                        <!-- Hidden Pattern Box -->
                        <div id="patternBox" class="hidden animate-fade-in-down">
                            <label class="block mb-2 text-sm font-medium text-slate-600 dark:text-slate-400">Pattern Code / Diagram</label>
                            <textarea name="pattern_code" placeholder="Enter pattern logic or diagram details..."
                                    class="w-full px-4 py-3 rounded-xl bg-slate-900 border border-slate-700 text-green-400 font-mono focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none transition-all shadow-sm min-h-[120px]"></textarea>
                            <p class="text-xs text-slate-500 mt-1">This field is only for pattern-based questions.</p>
                        </div>

                        <!-- Options Grid -->
                        <div>
                            <label class="block mb-2 text-sm font-medium text-slate-600 dark:text-slate-400">Answer Options</label>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="relative">
                                    <span class="absolute left-3 top-3.5 text-slate-400 text-sm font-bold">A</span>
                                    <input name="option1" placeholder="Option 1" required class="w-full pl-8 pr-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent outline-none transition-all">
                                </div>
                                <div class="relative">
                                    <span class="absolute left-3 top-3.5 text-slate-400 text-sm font-bold">B</span>
                                    <input name="option2" placeholder="Option 2" required class="w-full pl-8 pr-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent outline-none transition-all">
                                </div>
                                <div class="relative">
                                    <span class="absolute left-3 top-3.5 text-slate-400 text-sm font-bold">C</span>
                                    <input name="option3" placeholder="Option 3" required class="w-full pl-8 pr-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent outline-none transition-all">
                                </div>
                                <div class="relative">
                                    <span class="absolute left-3 top-3.5 text-slate-400 text-sm font-bold">D</span>
                                    <input name="option4" placeholder="Option 4" required class="w-full pl-8 pr-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent outline-none transition-all">
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Correct Answer -->
                        <div>
                            <label class="block mb-2 text-sm font-medium text-slate-600 dark:text-slate-400">Correct Answer</label>
                            <select name="correct_option" required class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 dark:focus:ring-emerald-400 focus:border-transparent outline-none transition-all shadow-sm">
    ...
    <script>
        ...
        function checkCategory() {
            ...
            const selText = select.selectedOptions[0].text.toLowerCase();
