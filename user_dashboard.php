...
// FORCE PROFILE CHECK 
$check = mysqli_query($conn, "SELECT id FROM user_profiles WHERE user_id = $user_id"); 
...
// Fetch profile data
$profile_query = mysqli_query($conn, "SELECT * FROM user_profiles WHERE user_id = $user_id");
$profile = mysqli_fetch_assoc($profile_query);

// 2. Optimized Category Fetching 
$sql = "SELECT c.*, COUNT(q.id) as question_count FROM categories c LEFT JOIN questions q ON c.id = q.category_id GROUP BY c.id ORDER BY c.id ASC"; 
...
<body class="bg-gray-50 min-h-screen relative" x-data="{ search: '', isDropdownOpen: false, categories: <?php echo htmlspecialchars(json_encode($categories)); ?> }"> 
        ...
        <div class="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between"> 
            ...
            <div class="hidden md:block flex-1 max-w-md mx-8"> 
                <div class="relative group"> 
                    ...
                    <input type="text" x-model="search" placeholder="Search topics..." class="w-full pl-10 pr-4 py-2.5 bg-gray-100 border-transparent rounded-xl focus:bg-white focus:border-green-300 focus:ring-4 focus:ring-green-100 transition-all outline-none text-sm"> 
                </div> 
            </div> 
            ...
            <div class="flex items-center gap-4"> 
                ...
                <div class="relative"> 
                    <button @click="isDropdownOpen = !isDropdownOpen" @click.outside="isDropdownOpen = false" class="flex items-center gap-3 pl-1 pr-1 py-1 rounded-full border border-gray-200 bg-white hover:shadow-md transition-all duration-200"> 
                        <img src="<?php echo htmlspecialchars($profile['avatar'] ?: 'img/loginlamge.png'); ?>" class="w-9 h-9 rounded-full object-cover border border-gray-100" /> 
                    ...
                    <div x-show="isDropdownOpen" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 translate-y-2" class="absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-xl border border-gray-100 py-2 z-[100] origin-top-right"> 
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10 animate-enter delay-100"> 
            <div class="lg:col-span-2 flex flex-col justify-center"> 
                ...
                <div class="md:hidden mt-4"> 
                    <input type="text" x-model="search" placeholder="Search categories..." class="w-full pl-4 pr-4 py-3 bg-white border border-gray-200 rounded-xl shadow-sm focus:border-green-400 outline-none text-gray-700"> 
                </div> 
            </div> 
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-enter delay-300"> 
            <template x-for="cat in categories.filter(c => c.name.toLowerCase().includes(search.toLowerCase()))" :key="cat.id"> 
                <div class="group relative bg-white/90 backdrop-blur-sm rounded-2xl p-6 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] hover:shadow-lg hover:-translate-y-1 transition-all duration-300 border border-gray-100"> 
                    ...
                    <a :href="'category.php?id=' + cat.id" class="inline-flex w-full items-center justify-center gap-2 px-4 py-2.5 bg-gray-900 text-white text-sm font-medium rounded-xl hover:bg-green-600 transition-colors shadow-lg shadow-gray-200 hover:shadow-green-200"> 
                        Start Practice <i class="ph ph-arrow-right"></i> 
                    </a> 
                </div> 
            </template> 

            <div x-show="categories.filter(c => c.name.toLowerCase().includes(search.toLowerCase())).length === 0" class="col-span-full py-16 text-center"> 
